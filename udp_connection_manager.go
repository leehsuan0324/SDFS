	...
	for {
		...
		if cnt == 3 {
			...
			for i := 1; i < len(servers); i++ {
				fmt.Printf("%v %v %v ", servers[i].Host, UCM.machine_id[i], test[UCM.alive_list[i]+5])
			}
			// fmt.Printf("\n====== Updated =======\n")
			cnt = 0
		}
	...
	for {
		...
		if UCM.alive_list[pos] >= 0 || UCM.alive_list[pos] == -2 {
			...
			if jg || UCM.alive_list[pos] == -2 {
				...
				if UCM.alive_list[pos] == 2 || UCM.alive_list[pos] == 0 {
...
func alive_list_update(pos int, status int8) {
	...
	case 1:
		if pos == _server.host_num && status == 1 {
			...
			UCM.machine_id[pos] = time.Now().String()
			UCM.alive_list_mutexs[pos].Unlock()
	...
	case 2:
		if pos != _server.host_num && (status == -5 || status == -4) {
			updated = true
			UCM.alive_list_mutexs[pos].Lock()
...
func udp_connection_management_init() {
	...
	UCM.machine_id = make([]string, len(servers))
	UCM.send_cnt = 0
	UCM.version = 0

	test = []string{"FAILURE", "FAILURE", "FAILURE", "LEAVING", "LEAVE", "JOINING", "JOINING", "RUNNING"}
